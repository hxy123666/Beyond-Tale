...
// 游戏配置
// ==============================
// 向上重力开关（true = 吸到天花板，角色倒立在天花板行走）
const GRAVITY_UP = true;
let TILE_SIZE = 32;
const GRAVITY = 0.52;
...
//         coyoteFrames: 0,        // 剩余土狼跳帧数
//         jumpBufferFrames: 0,     // 剩余跳跃缓冲帧数
//         canJump: true,
//         sprite: {
//             image: null,
    ...
    for (const savePoint of savePointLayer.objects) {
        ...
        if (playerLeft < spRight &&
            ...
            return {
                ...
                y: GRAVITY_UP ? (savePoint.y) : (savePoint.y - savePoint.height - player.height),
...
function updatePlayer() {
    ...
    if (gameState.keys[' ']) {
        if (!player.isJumping && player.canJump) {
            const jumpVel = GRAVITY_UP ? Math.abs(JUMP_FORCE) : JUMP_FORCE; // 正数 = 向下，负数 = 向上
            player.velY = jumpVel;
    ...
    // --- Y轴物理与碰撞 ---
    // 改为检测“头顶1px”是否有“地面”（即天花板）
    const groundCheck = checkCollision(player.x, player.y + (GRAVITY_UP ? -1 : +1), player.width, player.height);
    ...
    if (player.jumpBufferFrames > 0 && (isOnGround || player.coyoteFrames > 0)) {
        const jumpVel = GRAVITY_UP ? Math.abs(JUMP_FORCE) : JUMP_FORCE;
        player.velY = jumpVel;
        player.isJumping = true;
        player.isFalling = false;
        player.jumpFrameCounter = 0;
    ...
    // 若头顶就是“地面”（天花板）并且没在跳/落，贴住天花板保持静止
    if (groundCheck && player.velY <= 0 && !player.isJumping && !player.isFalling) {
        player.velY = 0;
        player.y = groundCheck.y + groundCheck.height; // 贴在天花板下缘
        ...
        if (player.isJumping) {
            ...
            // 跳跃位移：正常=上升为负，倒重力=向下为正
            const jumpDisp = GRAVITY_UP ? (player.y - player.jumpStartY) : (player.jumpStartY - player.y);

            // 手感：到达一定高度后减小重力（倒重力时：向下位移达到阈值）
            let currentGravity = GRAVITY;
            const isAscending = GRAVITY_UP ? (player.velY > 0) : (player.velY < 0); // 倒重力：往下是“上升阶段”
            if (jumpDisp >= JUMP_GRAVITY_REDUCTION_HEIGHT && isAscending && !player.hitCeiling) {
            ...
            if (player.hitCeiling) {
                ...
                if (remainingFrames <= 0) {
                    player.hitCeiling = false;
                    player.velY += GRAVITY_UP ? -GRAVITY : GRAVITY;
                } else {
                    player.velY = GRAVITY_UP ? -0.01 : 0.01;
                }
            } else {
                player.velY += GRAVITY_UP ? -currentGravity : currentGravity;
            }
        } else {
            // 自然受重力
            player.velY += GRAVITY_UP ? -GRAVITY : GRAVITY;
        ...
        // 何时进入“下落阶段”：原来 velY>=0，现在翻转为 velY<=0
        if (!player.isFalling && (GRAVITY_UP ? (player.velY <= 0) : (player.velY >= 0)) && player.isJumping) {
        ...
        if (yCollision) {
            const landing = GRAVITY_UP ? (player.velY < 0) : (player.velY > 0); // 倒重力：向上撞到天花板=着陆
            if (landing) {
                // 着陆：计算“坠落距离”（对称化）
                if (player.isFalling) {
                    const impactY = GRAVITY_UP ? (yCollision.y + yCollision.height) : (yCollision.y - player.height);
                    const fallDistance = GRAVITY_UP ? (player.fallStartY - impactY) : (impactY - player.fallStartY);
                    ...
                    if (tiles == 3) {
                        ...
                        player.y = impactY;
                        const v = Math.sqrt(2 * GRAVITY * 3.3 * TILE_SIZE);
                        player.velY = GRAVITY_UP ? +v : -v;
                        player.isJumping = true;
                        player.isFalling = false;
                        player.jumpFrameCounter = 0;
                        player.hitCeiling = false;
                        player.ceilingHangFrames = 0;
                        triggerBounceEffect(player.x + player.width / 2, GRAVITY_UP ? (yCollision.y + yCollision.height) : yCollision.y);
                        countJumps('jump43');
                        gameStateManager.saveToLocalStorage();
                        return;
                    } else if (tiles == 5) {
                        player.y = impactY;
                        const v = Math.sqrt(2 * GRAVITY * 4.3 * TILE_SIZE);
                        player.velY = GRAVITY_UP ? +v : -v;
                        player.isJumping = true;
                        player.isFalling = false;
                        player.jumpFrameCounter = 0;
                ...
                // 正常着陆到“地面”（倒重力=天花板）
                player.y = GRAVITY_UP ? (yCollision.y + yCollision.height) : (yCollision.y - player.height);
    ...
    if (player.y > mapHeight + 50 || player.y + player.height < -50) {
...
function updatePlayerAnimation() {
    ...
    let newAnimation;
    if (player.isJumping) {
        const isJumpPhase = GRAVITY_UP ? (player.velY > 0) : (player.velY < 0);
        newAnimation = isJumpPhase ? 'jump' : 'fall';
...
function setPlayerStartPosition() {
    ...
    if (gameState.lastCheckpoint) {
        ...
        if (collisionLayer) {
            ...
            const isSolid = (r, c) =>
                r >= 0 && r < h && c >= 0 && c < w && data[r * w + c] > 0;

            // 从右往左数第4列 -> 下标 = w - 4（1-based: 右边第1列= w-1, 第4列= w-4）
            const targetCol = Math.max(0, Math.min(w - 1, w - 4));

            // 先尝试在目标列找出生点
            const tryCol = (col) => {
                if (!GRAVITY_UP) {
                    // 正常重力：地面上（该格为实心，上方一格空）
                    for (let row = h - 1; row >= 0; row--) {
                        if (isSolid(row, col)) {
                            const aboveEmpty = row > 0 ? !isSolid(row - 1, col) : true;
                            if (aboveEmpty) {
                    ...
                    // 向上重力：天花板下（该格为实心，下方一格空）
                    for (let row = 0; row < h; row++) {
                        if (isSolid(row, col)) {
                            const belowEmpty = row < h - 1 ? !isSolid(row + 1, col) : true;
                            if (belowEmpty) {
                                p.x = col * TILE_SIZE + Math.floor((TILE_SIZE - p.width) / 2);
                                p.y = (row + 1) * TILE_SIZE; // 贴在天花板下缘
                                return true;
                            }
                        }
                    }
            ...
            // 1) 先试目标列
            spawnFound = tryCol(targetCol);

            // 2) 目标列不行 -> 从最右向最左兜底找一个最近可站位置
            if (!spawnFound) {
                for (let col = w - 1; col >= 0 && !spawnFound; col--) {
                    spawnFound = tryCol(col);
                }
            }
        }
        ...
        // 3) 完全找不到就兜底
        if (!spawnFound) {
            // 兜底放在右侧第4列的水平位置（尽量贴右侧）
            const mapW = (gameState.mapData?.width || 0) * TILE_SIZE;
            const desiredX = Math.max(0, mapW - 4 * TILE_SIZE);
            p.x = Math.floor(desiredX + (TILE_SIZE - p.width) / 2);
            p.y = 0;
        }
    }
...
function spawnBounceDust(cx, cy) {
    ...
    for (let i = 0; i < cfg.count; i++) {
        ...
        const vy = (GRAVITY_UP ? +1 : -1) * Math.abs(speed) * (0.2 + Math.random() * 0.6); // 倒重力：尘土向下
        const size = cfg.sizeMin + Math.random() * (cfg.sizeMax - cfg.sizeMin);
        const life = Math.floor(cfg.lifeMin + Math.random() * (cfg.lifeMax - cfg.lifeMin));
...
function triggerJumpEffect() {
    const p = gameState.player;
    const ground = checkCollision(p.x, p.y + (GRAVITY_UP ? -1 : +1), p.width, p.height);
    if (!ground) return;

    const cx = p.x + p.width / 2;
    const gy = GRAVITY_UP ? (ground.y + ground.height) : ground.y;
    spawnJumpDust(cx, gy);
...
function drawPlayer() {
    ...
    const scaleY = GRAVITY_UP ? -1 : 1; // 倒立
    ctx.scale(scaleX, scaleY);

    ctx.drawImage(
        sprite.image, sx, sy, frameWidth, frameHeight,
